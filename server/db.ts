...
type ResourceRow = {
  ...
  setor: string;
  bairros_json: string;
  militares_json: string;
  status: string;
  status_started_at: number;
  turno_inicio: string;
  turno_fim: string;
  restricao_empenho: number;
  restricao_empenho_motivo: string;
  current_occurrence_json: string | null;
};
...
function stampOnDate(date: string, time: string) {
...
function seedShiftIfNeeded(shiftDate: string) {
  const existing = db.prepare("SELECT COUNT(*) AS total FROM resources WHERE shift_date = ?").get(shiftDate) as {
    total: number;
  };
  ...
  db.transaction(() => {
    for (const resource of resources) {
      ...
      const info = insertResource.run(
        ...
        resource.cia,
        resource.setor,
        serialize(coverage?.bairros || []),
        serialize(resource.militares),
        resource.status,
        resource.statusStartedAt,
        resource.turnoInicio,
        resource.turnoFim,
        resource.restricaoEmpenho ? 1 : 0,
        resource.restricaoEmpenhoMotivo,
        serialize(resource.currentOccurrence),
        resource.statusStartedAt,
      );
      ...
      for (const historyEntry of resource.history) {
        insertHistory.run(
          ...
          serialize(historyEntry.details),
        );
      }
    }
...
export function getDashboard(shiftDate: string) {
  ...
  const resourceRows = db
    ...
    .all(shiftDate) as ResourceRow[];

  const historyRows = db
    .prepare(
      `SELECT id, resource_id, status, started_at, ended_at, details_json
       FROM resource_history
       WHERE resource_id IN (SELECT id FROM resources WHERE shift_date = ?)
       ORDER BY started_at ASC`,
    )
    .all(shiftDate) as Array<{
      id: number;
      resource_id: number;
      status: string;
      started_at: number;
  ...
  const historyByResource = new Map<number, unknown[]>();
  ...
  const resources = resourceRows.map((row) => ({
    ...
    cia: row.cia,
    setor: row.setor,
    bairros: deserialize<string[]>(row.bairros_json, []),
    militares: deserialize<string[]>(row.militares_json, []),
    status: row.status,
  ...
  const controls = db
    .prepare(
      `SELECT id, shift_date, tipo, militar, observacao, prefixo_previsto, prefixo_atual, created_at
       FROM controls
       WHERE shift_date = ?
  ...
  const escorts = db
    ...
    .map((row: any) => ({
      ...
      guarnicaoResponsavel: deserialize<string[]>(row.guarnicao_responsavel_json, []),
...
export function insertResource(input: {
  ...
  const info = db.prepare(
    ...
    input.turnoInicio,
    input.turnoFim,
    input.restricaoEmpenho ? 1 : 0,
    input.restricaoEmpenhoMotivo || "",
    null,
    createdAt,
  );
  ...
  db.prepare(
    `INSERT INTO resource_history (resource_id, status, started_at, ended_at, details_json)
     VALUES (?, ?, ?, ?, ?)`,
  ).run(Number(info.lastInsertRowid), input.status, createdAt, null, serialize({ note: "Recurso lançado no turno" }));
...
export function updateResourceStatus(
  resourceId: number,
  input: { status: string; natureza: string; endereco: string; breveHistorico: string; monitorar: boolean },
) {
  const startedAt = Date.now();
  const resource = db.prepare("SELECT id FROM resources WHERE id = ?").get(resourceId) as { id: number } | undefined;
  ...
  ).run(startedAt, resourceId);

  const occurrence =
    input.status === "Empenhada"
      ? {
  ...
  db.prepare(
    `UPDATE resources
     ...
     WHERE id = ?`,
  ).run(input.status, startedAt, serialize(occurrence), resourceId);
...
export function insertEscort(input: {
  ...
  const info = db.prepare(
    ...
    input.reds,
    input.natureza,
    serialize(guarnicaoResponsavel),
    input.responsavelExterno || "",
    input.prefixoApoio || "",
    inicioTs,
    input.umMilitarNaEscolta ? 1 : 0,
    input.militarEscolta || "",
    input.recursoLiberado ? 1 : 0,
    input.hospitalNome || "",
    input.origemTurnoData || "",
    input.observacao || "",
    input.status || "Ativa",
    createdAt,
  );
...
export function insertHighlight(input: {
  ...
  const createdAt = Date.now();
  const info = db.prepare(
    `INSERT INTO highlights (record_date, reds, categoria, natureza, resumo, created_at)
     VALUES (?, ?, ?, ?, ?, ?)`,
  ).run(input.recordDate, input.reds, input.categoria, input.natureza, input.resumo, createdAt);
...
export function insertAnnouncement(input: {
  ...
  const info = db.prepare(
    ...
    input.turnoNome,
    input.dataLabel,
    input.chamadaHorarios,
    input.lancamentoHorarios,
    input.viaturasConformeEscala,
    input.previstos,
    input.presentes,
    input.baixas,
    input.faltas,
  ...
  const insertItem = db.prepare(
    `INSERT INTO announcement_items (announcement_id, bloco, titulo, prefixo, efetivo, observacao, order_index)
     VALUES (?, ?, ?, ?, ?, ?, ?)`,
  );
...
export function copyPreviousShift(targetDate: string) {
  const sourceDate = previousDate(targetDate);
  const existingTarget = db.prepare("SELECT COUNT(*) AS total FROM resources WHERE shift_date = ?").get(targetDate) as {
    total: number;
  };
  if (existingTarget.total > 0) {
    return { ok: false as const, reason: "A data selecionada já possui viaturas cadastradas." };
  }

  const sourceResources = db
    .prepare("SELECT * FROM resources WHERE shift_date = ? ORDER BY created_at ASC")
    .all(sourceDate) as ResourceRow[];

  if (sourceResources.length === 0) {
    return { ok: false as const, reason: "Não existe turno anterior cadastrado para copiar." };
  }

  const insertResourceStatement = db.prepare(
    `INSERT INTO resources (
      shift_date, prefixo, tipo, cia, setor, bairros_json, militares_json, status,
      status_started_at, turno_inicio, turno_fim, restricao_empenho, restricao_empenho_motivo, current_occurrence_json, created_at
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
  ...
  db.transaction(() => {
    for (const resource of sourceResources) {
      const coverage = getCoverage(resource.cia, resource.setor);
      const info = insertResourceStatement.run(
        targetDate,
        resource.prefixo,
        resource.tipo,
        resource.cia,
        resource.setor,
        serialize(coverage?.bairros || deserialize<string[]>(resource.bairros_json, [])),
        resource.militares_json,
        "QCL",
        startedAt,
        resource.turno_inicio,
        resource.turno_fim,
        resource.restricao_empenho,
        resource.restricao_empenho_motivo,
        null,
        startedAt,
      );
      ...
      insertHistoryStatement.run(
        ...
        startedAt,
        null,
        serialize({ note: `Escala copiada de ${sourceDate}` }),
      );
    }
  })();

  return { ok: true as const, sourceDate, total: sourceResources.length };
}

export function buildFinalReport(recordDate: string) {
  const dashboard = getDashboard(recordDate);

  const escortsText =
    dashboard.escorts.length === 0
      ...
      : dashboard.escorts
          .map((escort, index) => {
            ...
            if (escort.hospitalNome) linhas.push(`Hospital: ${escort.hospitalNome}`);
            if (escort.umMilitarNaEscolta) linhas.push(`Militar em escolta: ${escort.militarEscolta || "não informado"}`);
            if (escort.recursoLiberado) linhas.push("Recurso liberado para atendimento: SIM");
            if (escort.origemTurnoData) linhas.push(`Origem do turno: ${escort.origemTurnoData}`);
            if (escort.observacao) linhas.push(`Obs: ${escort.observacao}`);
            return linhas.join("\n");
          })
          .join("\n\n");
  ...
  const highlightsText =
    dashboard.highlights.length === 0
      ...
      : dashboard.highlights
          .map(
            (item, index) =>
              `${index + 1}. [${item.categoria}] REDS ${item.reds || "não informado"} | ${item.natureza}\n${item.resumo}`,
          )
          .join("\n\n");
            ...
            const itemsText =
              announcement.items.length === 0
                ...
                : announcement.items
                    .map((item) => {
                      const parts = [];
                      if (item.bloco) parts.push(`*${item.bloco}*`);
                      if (item.titulo || item.prefixo) parts.push(`🚔 *${item.titulo}${item.prefixo ? ` - ${item.prefixo}` : ""}*`);
                      if (item.efetivo) parts.push(item.efetivo);
